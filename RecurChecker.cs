...
namespace LabCalculator
{
    class RecurChecker
    {
        public static bool Check(Cell currentCell, string currentVar, HashSet<string> vars)
        {
            try
            {
                /*
                ...
                */
                ...
                foreach (string cellName in cellsToGo)
                {
                    ...
                    if (!f)
                    {
                        f = f || Check((Cell)Program.form1.getDataGridView().Rows[numbers].Cells[letters], cellName, vars);
                    }
                }
                return f;
            }
